...
open class VerticalSeekBar @JvmOverloads constructor(
    ...
    private fun applyAttributes() {
        if (initEnded) {
            ...
            var thumbCardView: RelativeLayout? = null // nullable for customization
            try {
                thumbCardView = thumb.findViewById(R.id.thumbCardView)
            } catch (ignored: NoSuchFieldError) {
            }

            var thumbText :TextView? = null

            try {
                thumbText = thumb.findViewById(R.id.thumbText)
            } catch (ignored: NoSuchFieldError) {
            }
...
//            thumbCardView?.radius = thumbContainerCornerRadius.toFloat()
    ...
    private fun updateViews() {
        if (initEnded) post {
            ...
            barProgress.translationY =-(barBackground.height-(barBackground.height * (maxValue - progress) / maxValue).toFloat())
            invalidate()
        }
    }
